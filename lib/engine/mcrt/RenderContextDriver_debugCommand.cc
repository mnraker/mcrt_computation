...
RenderContextDriver::debugCommandParserConfigure()
{
    ...
    parser.opt("snapshotDeltaRec", "...command...", "snapshotDeltaRec command",
               [&](Arg &arg) { return mParserDebugCommandSnapshotDeltaRec.main(arg.childArg()); });
    parser.opt("showPrecision", "", "show AOV precision information",
               [&](Arg &arg) { return arg.msg(mFbSender.jsonPrecisionInfo() + '\n'); });
    ...
    parser.opt("renderContext", "...command...", "renderContext command",
               [&](Arg &arg) { return debugCommandRenderContext(arg); });
    parser.opt("fbSender", "...command...", "McrtFbSender command",
               [&](Arg& arg) { return mFbSender.getParser().main(arg.childArg()); });
    parser.opt("initFrameCtrl", "...command...", "initial frame control",
               [&](Arg& arg) { return mParserInitialFrameControl.main(arg.childArg()); });
    parser.opt("messageHistory", "...command...", "message history related command",
               [&](Arg& arg) { return mMessageHistory.getParser().main(arg.childArg()); });
    parser.opt("multiBank", "...command...", "multi-bank related command",
               [&](Arg& arg) { return mParserMultiBankControl.main(arg.childArg()); });
    parser.opt("logging", "...command...", "logging related command",
               [&](Arg& arg) { return mParserLoggingControl.main(arg.childArg()); });
    ...
    parser.opt("feedbackFb", "...command...", "feedback fb command",
               [&](Arg& arg) { return mFeedbackFb.getParser().main(arg.childArg()); });
    parser.opt("feedbackUpdate", "...command...", "feedback update command",
               [&](Arg& arg) { return mFeedbackUpdates.getParser().main(arg.childArg()); });
    ...
    parser.opt("feedbackStats", "", "show feedback statistical information",
               [&](Arg& arg) { return arg.msg(showFeedbackStats() + '\n'); });
    parser.opt("sentImageCache", "...command...", "sent image data cache command",
               [&](Arg& arg) { return mSentImageCache.getParser().main(arg.childArg()); });
    ...
    parser.opt("vecSender", "...command...", "McrtVecSender command",
               [&](Arg& arg) { return mVecSender.getParser().main(arg.childArg()); });
    parser.opt("pathVisMode", "<on|off|show>", "set pathVisualizer mode",
               [&](Arg& arg) {
                   moonray::rndr::PathVisualizerManager* visMgrObsrPtr =
                       getRenderContext()->getPathVisualizerManager().get();
                   if (arg() == "show") arg++;
                   else {
                       if ((arg++).as<bool>(0)) visMgrObsrPtr->turnOn();
                       else                     visMgrObsrPtr->turnOff();
                   }
                   return arg.msg(str_util::boolStr(visMgrObsrPtr->isOn()) + '\n');
               });
    ...
    parserSSR.opt("dump", "<filename>", "dump snapshotDelta to file",
                  [&](Arg &arg) {
                      const std::string filename = arg(0);
    ...
    parserIFC.opt("show", "", "show current condition",
                  [&](Arg& arg) { return arg.msg(showInitFrameControlStat() + '\n'); });
    ...
    parserMBC.opt("show", "", "show current condition",
                  [&](Arg& arg) { return arg.msg(showMultiBankControlStat() + '\n'); });
